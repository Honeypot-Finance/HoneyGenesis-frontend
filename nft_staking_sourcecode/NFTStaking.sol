    ...
    UUPSUpgradeable
{
    ...
    event StakedFor(address indexed owner, address indexed operator, uint256 indexed tokenId);
    event UnstakedFor(address indexed owner, address indexed operator, uint256 indexed tokenId);
    ...
    function _pendingAmount(
        ...
        if (s.burned) {
            ...
            uint256 lastBurnClaim = s.lastBurnClaimAt == 0 ? s.burnedAt : s.lastBurnClaimAt;
    ...
    function _accrueBase(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            uint256 epochEnd = i + 1 < len ? uint256(_paramEpochs[i + 1].start) : type(uint256).max;
    ...
    function _accrueBaseWithBps(
        ...
        if (len == 0) {
            ...
            return (rewardRatePerSecond * (endTs - startTs) * (MAX_BPS + burnBonusBps)) / MAX_BPS;
        }
        for (uint256 i = 0; i < len; i++) {
            ParamEpoch memory e = _paramEpochs[i];
            uint256 epochStart = uint256(e.start);
            ...
            sum += (e.rewardRatePerSecond * delta * (MAX_BPS + e.burnBonusBps)) / MAX_BPS;
        }
    }
    ...
    function batchStakeFor(address owner, uint256[] calldata tokenIds) external nonReentrant {
    ...
    function _claim(uint256 tokenId) internal returns (uint256 amount) {
        ...
        require(sStore.owner == msg.sender, "NOT_OWNER");
    ...
    function batchUnstakeFor(address owner, uint256[] calldata tokenIds) external nonReentrant {
        require(tokenIds.length > 0, "EMPTY_ARRAY");
        require(tokenIds.length <= 50, "TOO_MANY_TOKENS");
    ...
    function batchUnstakeForMultipleOwners(uint256[] calldata tokenIds) external nonReentrant {
        require(tokenIds.length > 0, "EMPTY_ARRAY");
